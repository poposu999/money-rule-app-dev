const CACHE_NAME = "money-rule-app-v47.32";
const APP_SHELL = ["./", "./index.html", "./style.css?v=47.32", "./app.js?v=47.32", "./manifest.json"];
...
self.addEventListener("fetch", event => {
  ...
  event.respondWith(fetch(event.request).then(async response => {
    ...
    if (path.endsWith("/index.html") || path.endsWith("/")) {
      ...
      let patched = text.replace(/Ver\.47\.\d+/g, "Ver.47.32").replace(/ver\.47\.\d+/g, "ver.47.32")
        .replace(/style\.css\?v=47\.\d+/g, "style.css?v=47.32").replace(/app\.js\?v=47\.\d+/g, "app.js?v=47.32");
