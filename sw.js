...
self.addEventListener("fetch", event => {
  ...
  event.respondWith(fetch(event.request).then(async response => {
    ...
    if (path.endsWith("/index.html") || path.endsWith("/")) {
      ...
      const patched = text
        .replace(/style\.css\?v=47\.45/g, "style.css?v=47.49")
        .replace(/style\.css\?v=47\.47/g, "style.css?v=47.49")
        .replace(/style\.css\?v=47\.48/g, "style.css?v=47.49")
        .replace(/app\.js\?v=47\.45/g, "app.js?v=47.49")
        .replace(/app\.js\?v=47\.47/g, "app.js?v=47.49")
        .replace(/app\.js\?v=47\.48/g, "app.js?v=47.49")
        .replace(/ver\.47\.45/g, "ver.47.49")
        .replace(/ver\.47\.46/g, "ver.47.49")
        .replace(/ver\.47\.47/g, "ver.47.49")
        .replace(/ver\.47\.48/g, "ver.47.49")
        .replace(/変更を保存/g, "保存")
        .replace(/<\/head>/i, `<style id="ver4749-table-style">${TABLE_STYLE}</style><script id="ver4749-edit-modal-script">${EDIT_MODAL_SCRIPT}</script>\n</head>`);
