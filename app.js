...
function hasMonthData(key){return monthExpenses(key).length>0||monthPendingPlanned(key).length>0||effectiveFixedEntries(key).length>0;}
...
function confirmDiscardUnsaved(){return !hasUnsavedChanges()||confirm("保存されていない変更があります。入力内容を破棄して月を移動しますか？");}
...
function confirmCrossMonth(source,dest,kind="項目"){
...
function updateDynamicTitles(){
  ...
  setText("spendingBudgetLabel",isCurrentSelected()?"今月使える予算":`${monthLabel(selectedMonth)}で使える予算`);
  setText("spentAmountLabel",isCurrentSelected()?"今月使った金額":`${monthLabel(selectedMonth)}に使った金額`);
  setText("plannedExpenseLabel",isCurrentSelected()?"今月の予定支出":`${monthLabel(selectedMonth)}の予定支出`);
  setText("fixedPendingLabel",isCurrentSelected()?"今月の未払い固定費":`${monthLabel(selectedMonth)}の未払い固定費`);
  setText("plannedSavingsLabel",isCurrentSelected()?"今月の貯金予定":`${monthLabel(selectedMonth)}の貯金予定`);
...
function switchMonth(next,{force=false}={}){
  if(!monthWithinAllowedRange(next))return;
  if(!force&&!confirmDiscardUnsaved())return;
...
function renderBudgetDashboard(){
  ...
  setPaceMeter(n.freeSpend,n.freeBudget);
...
function renderDailyChart(){
  const data=sourceDataForMonth(selectedMonth),tot=sourceTotals(data),days=daysInMonth(selectedMonth),vals=Array.from({length:days},()=>({actual:0,planned:0,fixed:0}));const add=(arr,type)=>arr.forEach(e=>{const d=Number(String(e.date||"").slice(8,10));if(d>=1&&d<=days)vals[d-1][type]+=Number(e.amount||0);});add(data.actual,"actual");add(data.planned,"planned");add(data.fixed,"fixed");const totals=vals.map(v=>v.actual+v.planned+v.fixed),max=Math.max(0,...totals),container=$("dailyChart");if(!max){container.innerHTML='<div class="chart-empty">表示できる支出データがありません</div>';return;}const labelDays=new Set([1,5,10,15,20,25,days]);container.innerHTML=legendHtml(tot)+`<div class="vertical-chart"><div class="vertical-bars">${vals.map((v,i)=>{const total=totals[i],h=max?total/max*100:0,a=total?v.actual/total*h:0,p=total?v.planned/total*h:0,f=total?v.fixed/total*h:0;return `<div class="bar-column" data-day="${i+1}"><div class="bar-stack" style="height:${h}%"><span class="bar-segment stack-actual" style="height:${total?v.actual/total*100:0}%"></span><span class="bar-segment stack-planned" style="height:${total?v.planned/total*100:0}%"></span><span class="bar-segment stack-fixed" style="height:${total?v.fixed/total*100:0}%"></span></div></div>`}).join("")}</div><div class="bar-labels">${vals.map((_,i)=>`<span class="bar-label">${labelDays.has(i+1)?i+1:''}</span>`).join("")}</div></div>`;container.querySelectorAll("[data-day]").forEach(el=>el.addEventListener("click",ev=>{const d=Number(el.dataset.day);popupBreakdown(`${monthLabel(selectedMonth)}${d}日`,vals[d-1],ev);}));
}
function renderMonthlyChart(){
  const keys=[];for(let i=5;i>=0;i--)keys.push(addMonths(selectedMonth,-i));const rows=keys.map(k=>{const v=sourceTotals(sourceDataForMonth(k));return {key:k,...v,total:v.actual+v.planned+v.fixed};}),max=Math.max(0,...rows.map(r=>r.total)),container=$("monthlyChart"),sum={actual:rows.reduce((s,r)=>s+r.actual,0),planned:rows.reduce((s,r)=>s+r.planned,0),fixed:rows.reduce((s,r)=>s+r.fixed,0)};if(!max){const msg=isFutureSelected()&&!monthExpenses(selectedMonth).length&&!monthPendingPlanned(selectedMonth).length&&!unpaidFixedEntries(selectedMonth).length?"予定されている支出はありません":"表示できる支出データがありません";container.innerHTML=`<div class="chart-empty">${msg}</div>`;return;}container.innerHTML=legendHtml(sum)+`<div class="vertical-chart"><div class="vertical-bars">${rows.map(r=>{const h=r.total/max*100;return `<div class="bar-column" data-month-bar="${r.key}"><div class="bar-stack" style="height:${h}%"><span class="bar-segment stack-actual" style="height:${r.total?r.actual/r.total*100:0}%"></span><span class="bar-segment stack-planned" style="height:${r.total?r.planned/r.total*100:0}%"></span><span class="bar-segment stack-fixed" style="height:${r.total?r.fixed/r.total*100:0}%"></span></div></div>`}).join("")}</div><div class="bar-labels">${rows.map(r=>`<span class="bar-label">${shortMonthLabel(r.key)}</span>`).join("")}</div></div>`;container.querySelectorAll("[data-month-bar]").forEach(el=>el.addEventListener("click",ev=>{const r=rows.find(x=>x.key===el.dataset.monthBar);popupBreakdown(monthLabel(r.key),r,ev);}));
}
...
function addExpense(){
  const amount=validateAmount($("expenseAmount").value),date=$("expenseDate").value,category=$("expenseCategory").value,memo=$("expenseMemo").value.trim();if(!amount){alert("金額を入力してください。");return;}if(!isValidDateString(date)||!dateAllowed(date)){alert("登録できる日付を確認してください。未来は現在月から1年先までです。");return;}const dest=monthOfDate(date);if(!confirmCrossMonth(selectedMonth,dest,"支出"))return;const e={id:newId("e"),amount,category:category||"その他",memo,date,order:nextOrderForDate(state.expenses,date),origin:null};state.expenses.push(e);state.memory.expenseCategory=category||"食費";rememberSessionDate("expense",date);save();clearDirty("expense-entry");if(dest!==selectedMonth){switchMonth(dest,{force:true});return;}$("expenseAmount").value="";$("expenseMemo").value="";$("expenseDate").value=sessionDefaultDate("expense",selectedMonth);renderAll();}
function addPlanned(){
  const amount=validateAmount($("plannedAmount").value),date=$("plannedDate").value,category=$("plannedCategory").value,memo=$("plannedMemo").value.trim();if(!amount){alert("金額を入力してください。");return;}if(!isValidDateString(date)||!dateAllowed(date)){alert("登録できる予定日を確認してください。未来は現在月から1年先までです。");return;}const dest=monthOfDate(date);if(!confirmCrossMonth(selectedMonth,dest,"予定支出"))return;const p={id:newId("p"),amount,category:category||"その他",memo,date,order:nextOrderForDate(state.plannedExpenses,date),status:"pending",confirmedExpenseId:null};state.plannedExpenses.push(p);state.memory.plannedCategory=category||"食費";rememberSessionDate("planned",date);save();clearDirty("planned-entry");if(dest!==selectedMonth){switchMonth(dest,{force:true});return;}$("plannedAmount").value="";$("plannedMemo").value="";$("plannedDate").value=sessionDefaultDate("planned",selectedMonth);renderAll();}
function addFixed(){
...
function unpayFixed(id,month=selectedMonth){const f=getFixedSource(id),info=fixedPaidInfo(f,month);if(!f||!info)return;const e=state.expenses.find(x=>String(x.id)===String(info.expenseId));if(e&&expenseChangedFromSnapshot(e)){if(!confirm("手動編集した支出も削除されます。実支出を削除して未払いへ戻しますか？"))return;}else if(!confirm("実支出を削除して未払いへ戻します。続行しますか？"))return;if(e)state.expenses=state.expenses.filter(x=>String(x.id)!==String(e.id));delete f.paidMonths[month];save();renderAll();}
function rollbackPlannedExpense(e){const p=getPlannedSource(e.origin?.sourceId);if(!p)return alert("元の予定支出が見つかりません。");if(!confirm("実支出を削除して予定支出へ戻します。続行しますか？"))return;p.amount=e.amount;p.category=e.category;p.memo=e.memo||"";p.date=e.date;p.order=nextOrderForDate(state.plannedExpenses,p.date);p.status="pending";p.confirmedExpenseId=null;state.expenses=state.expenses.filter(x=>String(x.id)!==String(e.id));rememberSessionDate("planned",p.date);save();closeEdit();const dest=monthOfDate(p.date);if(dest!==selectedMonth){switchMonth(dest,{force:true});return;}renderAll();}
function rollbackFixedExpense(e){const month=e.origin?.sourceMonth,f=getFixedSource(e.origin?.sourceId);if(!f||!month)return alert("元の固定費が見つかりません。");unpayFixed(f.id,month);closeEdit();if(month!==selectedMonth)switchMonth(month,{force:true});}
...
function saveExpenseEdit(){const e=state.expenses.find(x=>String(x.id)===String($("editModal").dataset.id));if(!e)return;const amount=validateAmount($("editAmount").value),date=$("editDate").value;if(!amount)return alert("金額を入力してください。");if(!isValidDateString(date)||!dateAllowed(date))return alert("日付を確認してください。");const sourceMonth=monthOfDate(e.date),dest=monthOfDate(date);if(!confirmCrossMonth(sourceMonth,dest,"支出"))return;e.amount=amount;e.category=$("editCategory").value;e.memo=$("editMemo").value.trim();if(e.date!==date)e.order=nextOrderForDate(state.expenses,date);e.date=date;if(e.origin?.type==="planned"||e.origin?.type==="fixed"){}rememberSessionDate("expense",date);save();closeEdit();if(dest!==selectedMonth){switchMonth(dest,{force:true});return;}renderAll();}
...
function savePlannedEdit(){const p=getPlannedSource($("plannedEditModal").dataset.id);if(!p)return;const amount=validateAmount($("plannedEditAmount").value),date=$("plannedEditDate").value;if(!amount)return alert("金額を入力してください。");if(!isValidDateString(date)||!dateAllowed(date))return alert("予定日を確認してください。");const source=monthOfDate(p.date),dest=monthOfDate(date);if(!confirmCrossMonth(source,dest,"予定支出"))return;p.amount=amount;p.category=$("plannedEditCategory").value;p.memo=$("plannedEditMemo").value.trim();if(p.date!==date)p.order=nextOrderForDate(state.plannedExpenses,date);p.date=date;rememberSessionDate("planned",date);save();closePlannedEdit();if(dest!==selectedMonth){switchMonth(dest,{force:true});return;}renderAll();}
...
function saveFixedEdit(){
  const f=getFixedSource($("fixedEditModal").dataset.id),old=configForFixedMonth(f,selectedMonth);if(!f||!old)return;const amount=validateAmount($("fixedEditAmount").value),category=$("fixedEditCategory").value,memo=$("fixedEditMemo").value.trim(),due=valueToDue($("fixedEditDue").value);if(!amount)return alert("金額を入力してください。");if(!category)return alert("カテゴリを選択してください。");if(!due)return alert("支払日を選択してください。");let effective=selectedMonth;const dueChanged=JSON.stringify(due)!==JSON.stringify(old.due);if(isPastSelected()&&dueChanged){if(confirm(`支払日の変更を${monthLabel(selectedMonth)}から反映しますか？\n\nOK：選択月から\nキャンセル：次の確認へ`)){effective=selectedMonth;}else if(confirm("今月から反映しますか？")){effective=currentMonthKey();}else return;}
  const later=f.changes.some(c=>compareMonth(c.effectiveMonth,effective)>0);if(later&&!confirm("この固定費には将来の変更予定があります。将来の予定を残したまま、この変更を追加しますか？"))return;
  upsertFixedChange(f,effective,{amount,category,memo,due});f.needsReview=false;
  const paid=fixedPaidInfo(f,selectedMonth);if(paid&&effective===selectedMonth){const e=state.expenses.find(x=>String(x.id)===String(paid.expenseId));if(e&&confirm("この月は支払済みです。選択月の確定済み支出にも変更を反映しますか？")){e.amount=amount;e.category=category;e.memo=memo;e.date=fixedDueDate(f,selectedMonth,{amount,category,memo,due});e.origin=e.origin||{type:"fixed",sourceId:f.id,sourceMonth:selectedMonth};e.origin.snapshot={amount:e.amount,category:e.category,memo:e.memo,date:e.date};}}
  save();closeFixedEdit();renderAll();
}
function deleteFixedFromMonth(){const f=getFixedSource($("fixedEditModal").dataset.id);if(!f)return;if(!confirm(`${monthLabel(selectedMonth)}以降の固定費設定を削除します。\n過去に確定した実支出は削除されません。続行しますか？`))return;f.endMonth=selectedMonth;f.changes=(f.changes||[]).filter(c=>compareMonth(c.effectiveMonth,selectedMonth)<0);save();closeFixedEdit();renderAll();}
...
function saveIncome(){const rec=selectedRecord();if(isPastSelected()&&!confirm("過去月の予算結果が変わります。収入・ボーナスを保存しますか？"))return;const incomeRaw=$("income").value,bonusRaw=$("bonus").value;if(incomeRaw!==""){const v=Number(incomeRaw);if(!Number.isFinite(v)||v<0)return alert("前月の収入を確認してください。");rec.income={entered:true,value:v};}if(bonusRaw!==""){const v=Number(bonusRaw);if(!Number.isFinite(v)||v<0)return alert("ボーナス額を確認してください。");rec.bonus={entered:true,value:v};}save();clearDirty("income");renderAll();}
function clearIncome(){if(!confirm("前月の収入を未入力状態に戻しますか？"))return;const rec=selectedRecord();rec.income={entered:false,value:0};save();suppressDirty=true;$("income").value="";suppressDirty=false;clearDirty("income");renderAll();}
function clearBonus(){if(!confirm("ボーナスを未入力状態に戻しますか？"))return;const rec=selectedRecord();rec.bonus={entered:false,value:0};save();suppressDirty=true;$("bonus").value="";suppressDirty=false;clearDirty("income");renderAll();}
function saveSettings(){const vals={minimumTakeHome:Number($("minimumTakeHome").value),savingsTarget:Number($("savingsTarget").value),extraAllowancePercent:Number($("extraAllowancePercent").value),extraSavingsPercent:Number($("extraSavingsPercent").value)};if(Object.values(vals).some(v=>!Number.isFinite(v)||v<0))return alert("家計ルールの入力内容を確認してください。");if(vals.extraAllowancePercent>100||vals.extraSavingsPercent>100)return alert("割合は0〜100%で入力してください。");if(isPastSelected()&&!confirm("過去月の予算結果が変わります。家計ルールを保存しますか？"))return;selectedRecord().settings=normalizeSettings(vals);selectedRecord().inheritedFrom=null;save();clearDirty("settings");setHidden("settingsMissing",true);renderAll();}
...
function collectIntegrityIssues(s){
  ...
  (s.expenses||[]).forEach(e=>{register("支出",e.id);if(!isValidDateString(e.date))issues.push(`支出 ${e.id}：不正な日付 ${e.date||'未入力'}`);if(e.origin?.type==="planned"&&!s.plannedExpenses.some(p=>String(p.id)===String(e.origin.sourceId)))issues.push(`支出 ${e.id}：元の予定支出が存在しません`);if(e.origin?.type==="fixed"&&!s.fixedExpenses.some(f=>String(f.id)===String(e.origin.sourceId)))issues.push(`支出 ${e.id}：元の固定費が存在しません`);});
  (s.plannedExpenses||[]).forEach(p=>{register("予定支出",p.id);if(!isValidDateString(p.date))issues.push(`予定支出 ${p.id}：不正な日付 ${p.date||'未入力'}`);if(p.status==="confirmed"&&!s.expenses.some(e=>String(e.id)===String(p.confirmedExpenseId)))issues.push(`予定支出 ${p.id}：確定先の実支出が存在しません`);});
...
function clearDevRestoreKeys(){[STORAGE_KEYS.state,STORAGE_KEYS.sections,STORAGE_KEYS.stats].forEach(k=>localStorage.removeItem(k));}
...
function handleBackupObject(data){
  ...
  if(v49raw!==undefined){let candidate;try{candidate=normalizeCurrentState(JSON.parse(v49raw));}catch{}const partialPrefs=()=>{const restored=[];[[STORAGE_KEYS.sections,"表示設定"],[STORAGE_KEYS.stats,"表示状態"]].forEach(([key,label])=>{if(ls[key]===undefined)return;try{const v=JSON.parse(String(ls[key]));if(v&&typeof v==="object"&&!Array.isArray(v))restored.push({key,label,value:String(ls[key])});}catch{}});return restored;};if(!candidate){const partial=partialPrefs();if(!partial.length)return alert("Ver.49データを検証できませんでした。復元できる部分もありません。現在のデータは変更していません。");if(!confirm(`Ver.49の家計簿本体は復元できません。\n復元可能：${partial.map(x=>x.label).join("・")}\n家計簿本体は変更せず、この部分だけ復元しますか？`))return;partial.forEach(x=>localStorage.setItem(x.key,x.value));location.reload();return;}const issues=collectIntegrityIssues(candidate);if(issues.some(x=>x.includes("不正な日付")||x.includes("IDがありません"))){const partial=partialPrefs();const details=issues.slice(0,5).join("\n");if(!partial.length)return alert(`バックアップ本体に復元できない問題があります。\n${details}\n復元できる部分もありません。現在のデータは変更していません。`);if(!confirm(`バックアップ本体に復元できない問題があります。\n${details}\n\n復元可能：${partial.map(x=>x.label).join("・")}\n家計簿本体は変更せず、この部分だけ復元しますか？`))return;partial.forEach(x=>localStorage.setItem(x.key,x.value));location.reload();return;}if(!confirm(`Ver.49形式のバックアップを復元します。\n確認事項：${issues.length}件\n現在の検証データを置き換えますか？`))return;clearDevRestoreKeys();localStorage.setItem(STORAGE_KEYS.state,JSON.stringify(candidate));if(ls[STORAGE_KEYS.sections]!==undefined)localStorage.setItem(STORAGE_KEYS.sections,String(ls[STORAGE_KEYS.sections]));if(ls[STORAGE_KEYS.stats]!==undefined)localStorage.setItem(STORAGE_KEYS.stats,String(ls[STORAGE_KEYS.stats]));localStorage.setItem(STORAGE_KEYS.migrated,"1");location.reload();return;}
  const legacyRaw=ls[LEGACY_STORAGE_KEYS.state]??ls[PROD_STORAGE_KEYS.state];if(legacyRaw===undefined){const restorable=[];if(ls[LEGACY_STORAGE_KEYS.sections]??ls[PROD_STORAGE_KEYS.sections])restorable.push("表示設定");if(ls[LEGACY_STORAGE_KEYS.stats]??ls[PROD_STORAGE_KEYS.stats])restorable.push("表示状態");if(!restorable.length)return alert("復元できる家計簿データがありません。現在のデータは変更していません。");if(!confirm(`家計簿本体は復元できません。${restorable.join('・')}だけ部分復元しますか？`))return;if(ls[LEGACY_STORAGE_KEYS.sections]??ls[PROD_STORAGE_KEYS.sections])localStorage.setItem(STORAGE_KEYS.sections,String(ls[LEGACY_STORAGE_KEYS.sections]??ls[PROD_STORAGE_KEYS.sections]));if(ls[LEGACY_STORAGE_KEYS.stats]??ls[PROD_STORAGE_KEYS.stats])localStorage.setItem(STORAGE_KEYS.stats,String(ls[LEGACY_STORAGE_KEYS.stats]??ls[PROD_STORAGE_KEYS.stats]));location.reload();return;}
  let legacy;try{legacy=JSON.parse(legacyRaw);}catch{return alert("旧形式データを解析できません。現在のデータは変更していません。");}const check=validateLegacyState(legacy);if(check.errors.length)return alert(`旧形式バックアップを変換できません。\n${check.errors.slice(0,6).map(x=>x.message).join('\n')}\n現在のデータは変更していません。`);if(!confirm("Ver.48系バックアップをVer.49形式へ変換して検証環境へ復元しますか？"))return;const converted=convertLegacyState(legacy);clearDevRestoreKeys();localStorage.setItem(STORAGE_KEYS.state,JSON.stringify(converted));localStorage.setItem(STORAGE_KEYS.migrated,"1");location.reload();
}
...
function bootstrap(){
  ...
  if(normalized){state=normalized;selectedMonth=currentMonthKey();ensureMonthRecord(selectedMonth);loadMonthForms();resetEntryTab();renderAll();if(sessionStorage.getItem("moneyRuleDevV49MigrationCompleted")==="1"){sessionStorage.removeItem("moneyRuleDevV49MigrationCompleted");setTimeout(()=>alert("Ver.49へのデータ移行が完了しました。"),50);}return;}
  const legacyRaw=readJson(LEGACY_STORAGE_KEYS.state,null);
  if(legacyRaw){$("migrationModal").classList.remove("hidden");setText("migrationStatus","移行前バックアップを保存してから開始してください。");return;}
  state=createEmptyState();state.months[currentMonthKey()]=createMonthRecord(null,null);save();localStorage.setItem(STORAGE_KEYS.migrated,"1");selectedMonth=currentMonthKey();loadMonthForms();resetEntryTab();renderAll();
}
bootstrap();
