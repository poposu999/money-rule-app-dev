...
function handleBackupObject(data){
  ...
  let legacy;try{legacy=JSON.parse(legacyRaw);}catch{return alert("旧形式データを解析できません。現在のデータは変更していません。");}const check=validateLegacyState(legacy);if(check.errors.length)return alert(`旧形式バックアップを変換できません。\n${check.errors.slice(0,6).map(x=>x.message).join('\n')}\n現在のデータは変更していません。`);if(!confirm("Ver.48系バックアップをVer.49形式へ変換して検証環境へ復元しますか？"))return;const converted=convertLegacyState(legacy);clearDevRestoreKeys();localStorage.setItem(STORAGE_KEYS.state,JSON.stringify(converted));localStorage.setItem(STORAGE_KEYS.migrated,"complete");location.reload();
}
